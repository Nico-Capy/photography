<template>
  <div class="flex flex-col items-center justify-center">
    ...
    >
      <div v-for="(photo, index) in photos" :key="index" class="relative cursor-pointer">
        <img
          :src="photo.src"
...
export default defineComponent({
  ...
  setup() {
    ...
    const photos = ref(
      Array.from({ length: 15 }, (_, i) => ({ src: `/pinhole${(i + 1).toString().padStart(2, "0")}.jpg` }))
    );
    ...
    onMounted(() => {
      ...
      // Animate heading
      gsap.from("h2", { opacity: 0, y: 50, duration: 0.8, ease: "power1.in" });
    });
